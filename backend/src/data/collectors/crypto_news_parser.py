...
def get_project_root():
    ...
    Учитывает текущее местоположение файла в структуре проекта.
    
    Returns:
        Path: Путь к корневой директории проекта
    """
    # Путь от текущего файла (collectors/crypto_news_parser.py) к корню проекта - 5 уровней вверх
    return Path(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))))
...
class CryptoNewsParser:
    ...
    def save_news(self, output_path, limit=100, pages=20, filename="crypto_news.csv"):
        ...
        if 'published_at' in combined_news.columns:
            try:
                combined_news['published_at'] = pd.to_datetime(combined_news['published_at'], format='ISO8601')
                combined_news.sort_values(by='published_at', ascending=False, inplace=True)
            except Exception as e:
                print(f"Ошибка при сортировке по дате: {e}")
                # Если и с ISO8601 возникла ошибка, пробуем формат 'mixed'
                try:
                    combined_news['published_at'] = pd.to_datetime(combined_news['published_at'], format='mixed')
                    combined_news.sort_values(by='published_at', ascending=False, inplace=True)
                    print("Сортировка выполнена успешно с использованием формата 'mixed'")
                except Exception as e2:
                    print(f"Не удалось выполнить сортировку даже с форматом 'mixed': {e2}")
...
def main():
    # Исправленный путь к конфигурационному файлу после перемещения в папку collectors
    base_dir = get_project_root()
    config_path = base_dir / "config" / "params.yml"
    
    print(f"Используется конфигурационный файл: {config_path}")
    
    # Путь для сохранения новостей с использованием относительного пути
    output_path = base_dir / "data" / "raw" / "news"
